...
@section('content')
    <div class="soft-card" style="max-width: 500px;">
        <div class="p-4">
            <form method="POST" action="{{ route('admin.barang-bukti.update', $barangBukti) }}">
                @csrf @method('PUT')
                <div class="mb-3">
                    <label class="form-label">Nama Barang Bukti</label>
                    <input type="text" name="nama" class="form-control" value="{{ old('nama', $barangBukti->nama) }}" required autofocus>
                </div>
